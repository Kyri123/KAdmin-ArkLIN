...
if ($serv->isInstalled() && $session_user->perm("$perm/home/admin_show")) {
    if (@file_exists($cheatfile)) {
        ...
        if(is_countable($arr) && is_array($arr) && count($arr) > 0) {
            for ($i=0;$i<count($arr);$i++) {
                if(trim($arr[$i]) !== "" && is_numeric(trim($arr[$i]))) {
                    ...
                    if($query->numRows() > 0) {
                        ...
                        $list_tpl->r("stname", isset($steamapi_user[$arr[$i]]["personaname"]) ? $steamapi_user[$arr[$i]]["personaname"] : "SteamAPI: ???");
                    ...
                    $list_tpl->r("url", "http://steamcommunity.com/profiles/".$arr[$i]);
                    $list_tpl->r("cfg", $serv->name());
                    $list_tpl->r("rndb", rndbit(25));
                    $list_tpl->r("img", isset($steamapi_user[$arr[$i]]["avatarmedium"]) ? $steamapi_user[$arr[$i]]["avatarmedium"] : "https://steamuserimages-a.akamaihd.net/ugc/885384897182110030/F095539864AC9E94AE5236E04C8CA7C2725BCEFF/");
    ...
    if($query->numRows() > 0)
        foreach ($query->fetchAll() as $item)
            if (!in_array(intval($item["SteamId"]), $arr) && trim($item["CharacterName"]) !== "")
                $userlist_admin .= "<option value='$item[SteamId]'>
                    ". (isset($steamapi_user[$item["SteamId"]]["personaname"]) ? $steamapi_user[$item["SteamId"]]["personaname"] : $item["SteamId"]) ." - $item[CharacterName]
                </option>";
} 
