...
// Cluster System
$json = $helper->file_to_json("data/panel/cluster_data.json");
...
foreach ($json as $k => $v) {
    ...
    if($json[$k]["sync"]["admin"] && $masterisset) {
        $mastercfg = file_get_contents($mcfg->get_save_dir(true)."/AllowedCheaterSteamIDs.txt");
        foreach ($json[$k]["servers"] as $sk => $sv) {
            if($sv["type"] != 1) {
                $serv = new server($sv["server"]);
                $file = $serv->get_save_dir(true)."/AllowedCheaterSteamIDs.txt";
    ...
    foreach ($json[$k]["servers"] as $sk => $sv) {
        ...
        if($changes) {
            $serv->cfg_save();
           if($ckonfig["clusterestart"] == 1) $serv->set_action("restart --warn --saveworld --noautoupdate", true);
        }
