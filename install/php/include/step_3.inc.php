...
if (isset($_POST["savepanel"])) {
    ...
    if ($KUTIL->filePutContents($ppath, $json_str)) {
        $link1 = false; $link2 = false;
        if ($KUTIL->filePutContents(__ADIR__."/remote/arkmanager/check", "done")) {
        ...
        if ($KUTIL->filePutContents(__ADIR__."/remote/serv/check", "done")) {
            unlink(__ADIR__."/remote/arkmanager/check");
            $link2  = true;
        }
        else {
